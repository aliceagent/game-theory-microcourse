#!/usr/bin/env python3
"""Assemble lesson 8 v3: unique monotonic shots, no picture loop, duration = narration."""
...
def make_title() -> Path:
    ...
    tab = (290, 88, 640, 248)
    tab_fill(tab, TEAL)
    x, y = ink_center(fb, "LESSON 08", tab)
    ImageDraw.Draw(img).text((x, y), "LESSON 08", font=fb, fill=PAPER)
    # Cream plate
    plate = (620, 255, 1075, 545)
    tab_fill(plate, PAPER)
    ImageDraw.Draw(img).rounded_rectangle(plate, 18, outline=(36, 51, 59, 40), width=2)
    t1 = "Parenting difference"
    t2 = "one: communication"
