#!/usr/bin/env python3
"""Assemble lesson 12 v3: unique monotonic shots, no picture loop, duration = narration."""
...
def make_title() -> Path:
    ...
    tab = (290, 88, 640, 248)
    tab_fill(tab, TEAL)
    x, y = ink_center(fb, "LESSON 12", tab)
    ImageDraw.Draw(img).text((x, y), "LESSON 12", font=fb, fill=PAPER)
    ...
    lines = [
        (ft, "Reinterpreting", SLATE),
        (fs, "the marshmallow test", TEAL),
        (fs, "as trust", TEAL),
    ]
