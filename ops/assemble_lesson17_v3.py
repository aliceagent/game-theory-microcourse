#!/usr/bin/env python3
"""Assemble lesson 17 v3: unique monotonic shots, no picture loop, duration = narration."""
...
def make_title() -> Path:
    ...
    tab = (290, 88, 640, 248)
    tab_fill(tab, TEAL)
    x, y = ink_center(fb, "LESSON 17", tab)
    ImageDraw.Draw(img).text((x, y), "LESSON 17", font=fb, fill=PAPER)
    ...
    lines = [
        (ft, "Parenting incentives", SLATE),
        (fs, "beyond the child", TEAL),
    ]
