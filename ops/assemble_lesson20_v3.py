#!/usr/bin/env python3
"""Assemble lesson 20 v3: unique monotonic shots, no picture loop, duration = narration."""
...
def make_title() -> Path:
    ...
    fb = ImageFont.truetype(FONT_B, 38)
    ft = ImageFont.truetype(FONT_B, 44)
    fs = ImageFont.truetype(FONT_R, 40)
    tab = (290, 88, 640, 248)
    tab_fill(tab, TEAL)
    x, y = ink_center(fb, "LESSON 20", tab)
    ImageDraw.Draw(img).text((x, y), "LESSON 20", font=fb, fill=PAPER)
    # Wider cream plate so the title stays inside the card.
    plate = (500, 210, 1288, 620)
    tab_fill(plate, PAPER)
    ImageDraw.Draw(img).rounded_rectangle(plate, 18, outline=(36, 51, 59, 40), width=2)
    pad = 28
    inner = (plate[0] + pad, plate[1] + pad, plate[2] - pad, plate[3] - pad)
    lines = [
        (ft, "Freedom, family", SLATE),
        (fs, "discussion and", TEAL),
        (fs, "storytelling", TEAL),
    ]
    for font, text, fill in lines:
        l, t, r, b = font.getbbox(text)
        if (r - l) > (inner[2] - inner[0]):
            raise SystemExit(f"TITLE_CLIP: {text!r} w={r-l} > plate inner {inner[2]-inner[0]}")
    h0, h1 = inner[1], inner[3]
    slot = (h1 - h0) / len(lines)
    for i, (font, text, fill) in enumerate(lines):
        box = (inner[0], h0 + i * slot, inner[2], h0 + (i + 1) * slot)
        x, y = ink_center(font, text, box)
