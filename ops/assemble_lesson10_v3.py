#!/usr/bin/env python3
"""Assemble lesson 10 v3: unique monotonic shots, no picture loop, duration = narration."""
...
def make_title() -> Path:
    ...
    tab = (290, 88, 640, 248)
    tab_fill(tab, TEAL)
    x, y = ink_center(fb, "LESSON 10", tab)
    ImageDraw.Draw(img).text((x, y), "LESSON 10", font=fb, fill=PAPER)
    ...
    lines = [
        (ft, "Feeling safe", SLATE),
        (fs, "and meeting authority", TEAL),
        (fs, "at school", TEAL),
    ]
