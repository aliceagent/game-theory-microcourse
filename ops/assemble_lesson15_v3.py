#!/usr/bin/env python3
"""Assemble lesson 15 v3: unique monotonic shots, no picture loop, duration = narration."""
...
def make_title() -> Path:
    ...
    tab = (290, 88, 640, 248)
    tab_fill(tab, TEAL)
    x, y = ink_center(fb, "LESSON 15", tab)
    ImageDraw.Draw(img).text((x, y), "LESSON 15", font=fb, fill=PAPER)
    ...
    lines = [
        (ft, "Why the lecturer turns", SLATE),
        (fs, "to social hierarchy", TEAL),
    ]
