#!/usr/bin/env python3
"""Assemble lesson 11 v3: unique monotonic shots, no picture loop, duration = narration."""
...
def make_title() -> Path:
    ...
    ft = ImageFont.truetype(FONT_B, 32)
    fs = ImageFont.truetype(FONT_R, 30)
    tab = (290, 88, 640, 248)
    tab_fill(tab, TEAL)
    x, y = ink_center(fb, "LESSON 11", tab)
    ImageDraw.Draw(img).text((x, y), "LESSON 11", font=fb, fill=PAPER)
    ...
    lines = [
        (ft, "Parenting difference", SLATE),
        (fs, "three: stability", TEAL),
        (fs, "and promises", TEAL),
    ]
