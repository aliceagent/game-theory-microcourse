#!/usr/bin/env python3
"""Render unique non-repeating H3 I2V shots for lesson 1 v3. One GPU at a time."""
...
V3 = ROOT / "ops/out/lesson-01/v3"
BEATS = json.loads((V3 / "beats.json").read_text())
H3 = "/home/nvidia/bin/h3-native.py"
MAX_CLIP_S = 7.0  # Jonathan 2026-09-09: no single clip longer than 7s


def h3_legal_duration(seconds: float) -> float:
    """Largest 24fps n%17==5 duration at or below min(requested, 7s)."""
...
ANCHORS = {
    ...
    "s2": V3 / "still-s2-new-h3.png",  # never reuse still-v2 / still-s2-h3 (already used for old i2v)
...
def claim_first_frame(path: Path, shot_id: int) -> None:
    """A still/first-frame hash may start exactly one I2V."""
    h = sha256(path)
    data = json.loads(USED.read_text()) if USED.exists() else {"used": {}}
    prev = data["used"].get(h)
    if prev and prev.get("shot") != shot_id:
        raise SystemExit(f"REFUSE: first-frame {path} hash already used for shot {prev}")
    data["used"][h] = {"shot": shot_id, "path": str(path)}
    USED.write_text(json.dumps(data, indent=2) + "\n")
...
def main():
    ...
    for ch in BEATS["chapters"]:
        ...
        for beat in ch["beats"]:
            ...
            claim_first_frame(Path(first_frame), bid)
            prompt = (
                BEATS["style"]
                + ". "
                + beat["motion"]
